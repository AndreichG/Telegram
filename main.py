...
async def fourth_response(update, context):
    if update.message.text == 'Играть':
        reply_keyboard = [['Все города'], ["Столицы Европы"],  ["Россия"], ["Большие города России"], ["Маленькие города России"]]
...
async def second_response(update, context):
    ...
    result2 = cur.execute(
        f"""SELECT points from stats WHERE username = \"{update.message.chat.username}\"""").fetchall()
    result3 = cur.execute(
        f"""SELECT city from stats WHERE username = \"{update.message.chat.username}\"""").fetchall()
    result4 = cur.execute(
        f"""SELECT mode from stats WHERE username = \"{update.message.chat.username}\"""").fetchall()
    diff = result1[0][0]
    points = result2[0][0]
    city = result3[0][0]
    mode = result4[0][0]
    ...
    if update.message.text == city:
        ...
        result = cur.execute(
            f"""UPDATE stats
                ...
                WHERE username = \"{update.message.chat.username}\"""").fetchall()
        con.commit()
        await update.message.reply_text(f'Вы угадали. Вам было выдано {round(dp, 2)} баллов\nТеперь у вас {round(points + dp, 2)} баллов', reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
    else:
        dp = 1
        result = cur.execute(
            f"""UPDATE stats
